...
void main()
{
  ...
  vec3 eye = vec3(sin(u_time), 8.0 * sin(u_time), 10.0 * cos(u_time));

  mat4 viewToWorld = makeViewMatrix(eye, vec3(0.0, 0.0, 0.0), vec3(0.0, 1.0, 0.0));
  vec3 worldDir = (viewToWorld * vec4(viewDir, 0.0)).xyz;
  float dist = shortestDistanceToSurface(eye, worldDir, MIN_DIST, MAX_DIST);

  gl_FragColor = vec4(0.1/dist * (sin(u_time + 1.2) + 0.9));
}
